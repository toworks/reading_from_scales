...
func (c *Config) Processing_schenck(message string) string {

  REQUEST := fmt.Sprintf("%s%s%s%s%s%s", `00`, `#`, c.Command, `#`, string(DLE), string(ETX))
  bcc, _ := hash_bcc([]byte(REQUEST))
  message = fmt.Sprintf("%s%s%s", string(STX), REQUEST, bcc)
  
  if DEBUG.enable {
    c.ch_message <- fmt.Sprintf("d|:|%s: Processing Schenck message: message: '%v'", mod_name, message)
  }
/*
...
func hash_bcc (buf []byte) (string, error) {
	total := buf[0]
	//	for i, v := range buf {
	for i := 1; i < len(buf); i++ {
		//*b ^= BCC(v)
